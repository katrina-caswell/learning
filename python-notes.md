...
# Note double brackets for selecting multiple columns
...
Lambda
An anonymous function (no name for defining it) for calculus - it is contained in one line of code
```
lambda argument(s): expression
# A colon separates function from expression and is used to represent an indented block
```

Merge/merge_ordered
```
pd.merge_ordered(x,y,on='date',suffixes=('_x','_y'),fill_method='ffill')
# Forward filling - fills blanks with previous value
# Can have multiple ons if you structure like this on=['date','country']
```
...
# See all rows from left visa table, values from ibm table are based on how close the date_time values match with the visa table
# Forward selects the first table of the right table whose on key column is greater than or equal to the left key column
```
...
Melt - unpivot a table from wide to long
Wide = every row relates to one subject, and each column has different information about an attribute of that subject
Long = information about one subject is found over many rows, and each row has one attribute about that subject
